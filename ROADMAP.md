...
- ✅ Refactor `server.py`: async `httpx`, `lifespan` startup, Pydantic-settings env config,
  dependency injection, input validation, CORS, `/stats` endpoint, graceful zero-retrieval.
  Deferred: rate limiting + retry / request-IDs.
